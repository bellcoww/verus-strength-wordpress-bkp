	...
	public function widget( $args, $instance ) {
		...
		do
		{
			...
			foreach ($classes as $class)
			{
				...
				if (array_key_exists(sanitize_title_with_dashes($class->ProgramName),$prg_url))
				{
					$programurl=$prg_url[sanitize_title_with_dashes($class->ProgramName)];
				}
				else
				{
					$programurl='';
				...
				if (!$programinactive)
				{
					...
					if (''!=$filtercoach)
					{
						...
						if ($class->Coaches)
						{
							...
							foreach ($coaches as $coach) {
								if ($coach->APICoach->Name == $filtercoach)
								{
									$docoaches = true;
									break 1;
								}	
					...
					{
								$output .= '<li>';
								if (array_key_exists(sanitize_title_with_dashes($class->ProgramName),$prg_bgcolors))
									$classcolor=$prg_bgcolors[sanitize_title_with_dashes($class->ProgramName)];
								else
									$classcolor = null;
								if (!$classcolor)
									$classcolor = '#'.$class->CalendarColor; 
									...
									switch ($line)
									{
										...
										case 'reservation':
											if ('true'==$showreservations)
											{
												...
												{
													$output .= $classlimit-$available.'/'.$class->ClassLimit. ' '.__('spaces reserved.','wp-wdfy-integration-of-wodify');
												}
												$output.= "</span>";
											}
											break;
