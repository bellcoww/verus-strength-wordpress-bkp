	...
	<?php
		...
		<section class="wdfysection_welcome">
		 <h2><?php _e('About this plugin','wp-wdfy-integration-of-wodify');?></h2>
			 ...
			 <p><?php _e('This plugin is provided by <a href="http://amrap42.28a.de" target="_blank">AMRAP42 - Stefan Osterburg</a>.','wp-wdfy-integration-of-wodify');?></p>
		...
		?>
		  ...
		  <tr valign="top">
        <th scope="row"><?php _e('Schema.org site image URL','wp-wdfy-integration-of-wodify');?></th>
		<?
		...
		<?php _e('If you use the [wdfyevents] shortcode or editor block this will create a schema.org event markup for search engines. Please enter a fully qualified URL (inkl. http/https of an image that can be used for this event markup. You can also chose to set individual URLs for each program on the Programs settings page.','wp-wdfy-integration-of-wodify'); ?>
		</td></tr>
			...
			<table class="wp-list-table striped"><tr><td><strong><?php echo __('Coach name','wp-wdfy-integration-of-wodify');?></strong></td><td><strong><?php _e('Coach About Page URL','wp-wdfy-integration-of-wodify');?></strong></td><tr><tr><?php
				if (is_array($coaches))
				{
					...
					{					
						...
						{
							$coachurl=$coach_url[sanitize_title_with_dashes($coach->Name)];
						}
						else
						{
							$coachurl='';
						...
						echo '<td>';?>							
						<input name="<?php 
									echo 'wdfy_coach_url['.sanitize_title_with_dashes($coach->Name).']'; 
							?>" type="text" size="50" value="<?php 
									echo $coachurl;
							?>"/></td>
						<?php
		...
		<?php _e('If you intend to use schema.org markup, please make sure to setup the site image and/or program image URLs in the plugin settings.','wp-wdfy-integration-of-wodify');?>
		</li>
