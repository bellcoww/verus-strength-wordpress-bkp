...
/**
 * Class Crossarena_News_Smart_Box_Widget
 */
class Crossarena_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'crossarena_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'crossarena' );
		$this->widget_description = esc_html__( 'News Smart Box', 'crossarena' );
		$this->utility            = crossarena_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'crossarena' ),
				'label' => esc_html__( 'Title', 'crossarena' ),
			),
			'layout_type' => array(
				'type'    => 'radio',
				'value'   => 'layout_type_1',
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'crossarena' ),
						'img_src' => CROSSARENA_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'crossarena' ),
						'img_src' => CROSSARENA_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'crossarena' ),
						'img_src' => CROSSARENA_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'crossarena' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
				'options' => array(
					'category' => array(
						'label' => esc_html__( 'Category', 'crossarena' ),
						'slave' => 'terms_type_category',
					),
					'post_tag' => array(
						'label' => esc_html__( 'Tag', 'crossarena' ),
						'slave' => 'terms_type_post_tag',
					),
				),
				'label' => esc_html__( 'Choose taxonomy type', 'crossarena' ),
			),
			'current_category' => array(
				...
				'options_callback' => array( $this, 'get_terms_list',
					array( 'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select category', 'crossarena' ),
				'placeholder'      => esc_html__( 'Select category', 'crossarena' ),
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'crossarena' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'crossarena' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'crossarena' ),
				'placeholder'      => esc_html__( 'Select tag', 'crossarena' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'crossarena' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select tags', 'crossarena' ),
				'master'           => 'terms_type_post_tag',
			),
			'posts_per_page' => array(
				...
				'max_value' => 20,
				'min_value' => 1,
				'label'     => esc_html__( 'Posts count', 'crossarena' ),
			),
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'crossarena' ),
			),
			'trim_content_words' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Content words trimmed count', 'crossarena' ),
			),
			'current_title_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display current navigation title', 'crossarena' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'crossarena' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'crossarena' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'crossarena' ),
			),
			'more_button_visibility' => array(
				'type'   => 'switcher',
				'value'  => 'true',
				'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label'  => esc_html__( 'Display more button', 'crossarena' ),
				'toggle' => array(
					'true_toggle'  => esc_html__( 'On', 'crossarena' ),
					'false_toggle' => esc_html__( 'Off', 'crossarena' ),
			...
			'more_button_text' => array(
				'type'   => 'text',
				'value'  => esc_html__( 'Read more', 'crossarena' ),
				'label'  => esc_html__( 'More button text', 'crossarena' ),
		...
		add_filter( 'crossarena_theme_script_variables', array( $this, 'add_new_smart_box_nonce_var' ) );
	...
	public function add_new_smart_box_nonce_var( $vars = array() ) {
		$vars['new_smart_box_nonce'] = wp_create_nonce( 'crossarena-new-smart-box-nonce' );
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'crossarena_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'crossarena' ) . '</h4></div>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$comments = $this->utility->meta_data->get_comment_count(array(
				...
				'sufix'   => get_comments_number_text( esc_html__( 'No comment(s)', 'crossarena' ), esc_html__( '1 comment', 'crossarena' ), esc_html__( '% comments', 'crossarena' ) ),
	...
	public function new_smart_box_instance(){
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'crossarena-new-smart-box-nonce' ) ) {
			exit();
		}
...
add_action( 'widgets_init', 'crossarena_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function crossarena_register_news_smart_box_widgets() {
	register_widget( 'Crossarena_News_Smart_Box_Widget' );
}
