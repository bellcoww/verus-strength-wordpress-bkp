...
if ( ! class_exists( 'Crossarena_About_Widget' ) ) {

	/**
	 * Class Crossarena_About_Widget.
	 */
	class Crossarena_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'crossarena' );
			$this->widget_id          = 'crossarena_widget_about';
			$this->widget_name        = esc_html__( 'About Crossarena', 'crossarena' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'crossarena' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'crossarena' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'crossarena' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'crossarena' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'crossarena' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'crossarena' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, crossarena_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'crossarena_widget_about_template_file', 'inc/widgets/about/views/about.php' ), false, false );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'crossarena_about_widget_image_size', 'crossarena-thumb-m' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'crossarena_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'crossarena_register_about_widget' );
/**
 * Register about widget.
 */
function crossarena_register_about_widget() {
	register_widget( 'Crossarena_About_Widget' );
}
