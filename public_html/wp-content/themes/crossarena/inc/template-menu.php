 ...
 * @return string
 */
function crossarena_get_main_menu() {
	$args = apply_filters( 'crossarena_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'crossarena_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'crossarena' ),
	) );
 ...
 * @return void
 */
function crossarena_main_menu() {

	$main_menu = crossarena_get_main_menu();
 ...
 * @return void
 */
function crossarena_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'crossarena_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="fa fa-chevron-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'crossarena' ) . '</span>',
		'button-close-inner-html' => '<i class="fa fa-times"></i>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = crossarena_get_main_menu();
 ...
 * @return void
 */
function crossarena_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', crossarena_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'crossarena_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'crossarena_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'crossarena' ),
	) );
 ...
 * @return void
 */
function crossarena_top_menu() {
	
	if ( ! get_theme_mod( 'top_menu_visibility', crossarena_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		return;
	}
	...
	wp_nav_menu( apply_filters( 'crossarena_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
		'container_class' => 'top-panel__menu',
		'menu_class'      => 'top-panel__menu-list inline-list container',
		'depth'           => 1,
	) ) );
}
 ...
 * @return string
 */
function crossarena_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'crossarena_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'crossarena_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'crossarena' ),
	), $context, $type );
 ...
 * @return null|string
 */
function crossarena_set_nav_menu( $args ) {
 ...
 * @return void
 */
function crossarena_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'crossarena_menu_toggle_html',
 ...
 * @return void
 */
function crossarena_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'crossarena_vertical_menu_toggle_html',
		'<div class="vertical-menu-toggle-holder"><button class="vertical-menu-toggle menu-toggle" aria-controls="%s" aria-expanded="false"><span class="menu-toggle-box"><span class="menu-toggle-inner"></span></span></button></div>'
	);
