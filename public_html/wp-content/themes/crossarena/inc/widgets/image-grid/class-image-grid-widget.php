 ...
 * @package Crossarena
 */
if ( ! class_exists( 'Crossarena_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Crossarena_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'crossarena' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'crossarena' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'crossarena' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'crossarena' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'crossarena' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'crossarena' ),
						'title'         => esc_html__( 'Post Title', 'crossarena' ),
						'comment_count' => esc_html__( 'Comment Count', 'crossarena' ),
					),
					'label'   => esc_html__( 'Post sorted', 'crossarena' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'crossarena' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'crossarena' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'crossarena' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'crossarena_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'crossarena_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'crossarena_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'crossarena_image_grid_widget_before',
				...
				echo apply_filters( 'crossarena_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'crossarena_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function crossarena_register_image_grid_widget() {
		register_widget( 'Crossarena_Image_Grid_Widget' );
	}
}
