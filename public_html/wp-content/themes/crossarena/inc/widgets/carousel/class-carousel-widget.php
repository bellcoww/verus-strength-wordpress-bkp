 ...
 * @package Crossarena
 */
if ( ! class_exists( 'Crossarena_Carousel_Widget' ) ) {

	/**
	 * Class Crossarena_Carousel_Widget.
	 */
	class Crossarena_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'crossarena' );
			$this->widget_id          = 'crossarena_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'crossarena' );
			...
			$this->settings           = array(
				...
				'tags' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'crossarena' ),
					'multiple'         => true,
				...
				'post_title' => array(
					...
					'label' => esc_html__( 'Display title', 'crossarena' ),
				),
				'content' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display content', 'crossarena' ),
				),
				'more_button' => array(
					'type'   => 'switcher',
					'value'  => 'false',
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 5,
					'label'     => esc_html__( 'Number of slides per view', 'crossarena' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'crossarena' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'crossarena' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'crossarena' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'crossarena' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'crossarena' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'crossarena' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'crossarena_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', crossarena_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="crossarena-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'crossarena' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'crossarena_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'crossarena' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$image = $this->utility->media->get_image( array(
					'size'              => 'post-thumbnail',
					'mobile_size'       => 'crossarena-thumb-m',
	...
	add_action( 'widgets_init', 'crossarena_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function crossarena_register_carousel_widget() {
		register_widget( 'Crossarena_Carousel_Widget' );
	}
}
