...
if ( ! class_exists( 'Crossarena_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Crossarena_Playlist_Slider_Widget.
	 */
	class Crossarena_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'crossarena' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'crossarena' );
			$this->widget_id          = apply_filters( 'crossarena_playlist_slider_widget_ID', 'crossarena_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'crossarena_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = crossarena_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'crossarena' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'crossarena' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'crossarena' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'crossarena' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'crossarena' ),
						true  => esc_html__( 'Buttons', 'crossarena' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'crossarena' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'crossarena' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'crossarena' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'crossarena' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'crossarena' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'crossarena' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'crossarena' ),
						'author'   => esc_html__( 'Author', 'crossarena' ),
						'comments' => esc_html__( 'Comment count', 'crossarena' ),
						'category' => esc_html__( 'Category', 'crossarena' ),
						'post_tag' => esc_html__( 'Tag', 'crossarena' ),
					),
					'label' => esc_html__( 'Display post meta data', 'crossarena' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'crossarena' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'crossarena_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'crossarena-thumb-s',
					'mobile_size' => 'crossarena-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'crossarena_playlist_buttons', false ),
...
add_action( 'widgets_init', 'crossarena_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function crossarena_register_playlist_slider_widget() {
	register_widget( 'Crossarena_Playlist_Slider_Widget' );
}
