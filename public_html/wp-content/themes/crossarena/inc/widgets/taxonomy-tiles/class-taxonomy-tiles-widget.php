...
if ( ! class_exists( 'Crossarena_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Crossarena_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'crossarena' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'crossarena' );
			$this->widget_id          = 'crossarena-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'crossarena' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'crossarena' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'crossarena' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'crossarena' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'crossarena' ),
						),
					),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'crossarena' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'crossarena_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'crossarena_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'crossarena' ),
				...
				echo apply_filters( 'crossarena_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'crossarena_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function crossarena_register_taxonomy_tiles_widget() {
		register_widget( 'Crossarena_Taxonomy_Tiles_Widget' );
	}
}
