 ...
 * @package Crossarena
 */
class Crossarena_Subscribe_Follow_Widget extends Cherry_Abstract_Widget
{
    ...
    public function __construct()
    {
        ...
        $this->widget_cssclass = 'crossarena widget-subscribe';
        $this->widget_description = esc_html__('Display subscribe form and follow links.', 'crossarena');
        $this->widget_id = 'crossarena_widget_subscribe_follow';
        $this->widget_name = esc_html__('Subscribe and Follow', 'crossarena');
        $this->settings = array(
            'enable_subscribe' => array(
                ...
                'options' => array(
                    'enable_subscribe' => esc_html__('Enable Subscribe Box', 'crossarena'),
                ),
            ),
            'subscribe_title' => array(
                'type' => 'text',
                'value' => esc_html__('Subscribe', 'crossarena'),
                'label' => esc_html__('Subscribe Title', 'crossarena'),
            ),
            'subscribe_message' => array(
                'type' => 'textarea',
                'label' => esc_html__('Subscribe text message', 'crossarena'),
            ),
            'subscribe_input' => array(
                'type' => 'text',
                'value' => esc_html__('Enter your email', 'crossarena'),
                'label' => esc_html__('Subscribe input placeholder', 'crossarena'),
            ),
            'subscribe_submit' => array(
                'type' => 'text',
                'value' => esc_html__('Subscribe', 'crossarena'),
                'label' => esc_html__('Subscribe submit label', 'crossarena'),
            ),
            'subscribe_success' => array(
                'type' => 'text',
                'value' => esc_html__('You successfully subscribed', 'crossarena'),
                'label' => esc_html__('Subscribe success', 'crossarena'),
            ),
            'enable_follow' => array(
                'type' => 'checkbox',
                'value' => array(
                ...
                'options' => array(
                    'enable_follow' => esc_html__('Enable Follow Box', 'crossarena'),
                ),
            ),
            'follow_title' => array(
                'type' => 'text',
                'value' => esc_html__('Follow', 'crossarena'),
                'label' => esc_html__('Follow Title', 'crossarena'),
            ),
            'follow_message' => array(
                'type' => 'textarea',
                'label' => esc_html__('Follow text message', 'crossarena'),
            ),
        );

        add_action('wp_ajax_crossarena_subscribe', array($this, 'process_subscribe'));
        add_action('wp_ajax_nopriv_crossarena_subscribe', array($this, 'process_subscribe'));

        $this->errors = array(
            'nonce' => esc_html__('Security validation failed', 'crossarena'),
            'mail' => esc_html__('Please, provide valid mail', 'crossarena'),
            'mailchimp' => esc_html__('Please, set up MailChimp API key and List ID', 'crossarena'),
            'internal' => esc_html__('Internal error. Please, try again later', 'crossarena'),
        );
    ...
    public function widget($args, $instance)
    {
        ...
        if ($subscribe_enabled && $subscribe_template && $api_key && $list_id) {
            ...
            esc_html_e('Please set up MailChimp API key and List ID', 'crossarena');
        }
    ...
    {
        return crossarena_get_social_list('widget');
    }
    ...
    public function get_subscribe_submit($class)
    {
        ...
        $subscribe_submit = crossarena_render_icons($subscribe_submit);
    ...
    public function process_subscribe()
    {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'crossarena_subscribe')) {
...
add_action('widgets_init', 'crossarena_register_subscribe_follow_widgets');
function crossarena_register_subscribe_follow_widgets()
{
    register_widget('Crossarena_Subscribe_Follow_Widget');
}
