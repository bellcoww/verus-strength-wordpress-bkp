 ...
 * @package Crossarena
 */
if ( ! class_exists( 'Crossarena_About_Author_Widget' ) ) {

	/**
	 * Class Crossarena_About_Author_Widget.
	 */
	class Crossarena_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'crossarena widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'crossarena' );
			$this->widget_id          = 'crossarena_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'crossarena' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'crossarena' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'crossarena' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'crossarena' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'crossarena' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'crossarena' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'crossarena' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'crossarena' ),
					'label' => esc_html__( 'Link label', 'crossarena' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'crossarena' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'crossarena-author-avatar' );
		...
		public function get_author_button() {
			...
			return sprintf(
				'<div class="about-author_btn_box"><a href="%2$s" class="%3$s">%1$s<i class="linearicon linearicon-arrow-right"></i></a></div>',
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'crossarena_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'crossarena_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function crossarena_register_about_author_widgets() {
	register_widget( 'Crossarena_About_Author_Widget' );
}
